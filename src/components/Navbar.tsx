...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-sm" role="navigation" aria-label="Main navigation">
      <div className="container flex items-center justify-between py-4 md:py-5 px-5 sm:px-6">
        <a href="#" className="text-sm font-black uppercase tracking-widest" aria-label="Lasha Uchaneishvili – Home">
          Lasha U.
        </a>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border bg-background animate-fade-in">
          <div className="container py-6 px-5 sm:px-6 flex flex-col gap-4">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
